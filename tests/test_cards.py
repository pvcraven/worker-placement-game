...
def test_all_cards_have_minimum_benefit(contracts):
    ...
    bad = [
        (c["name"], _benefit(c))
        for c in contracts
        if _benefit(c) < min_benefit
    ]
    assert not bad, (
        f"Cards with benefit < {min_benefit}: "
        + ", ".join(
...
def test_benefit_not_more_than_three_times_cost(contracts):
    ...
    for c in contracts:
        ...
        if benefit > cost * 3:
            bad.append(
                (c["name"], cost, benefit,
                 f"{benefit / cost:.1f}x" if cost else "inf")
            )
    assert not bad, (
        "Cards with benefit > 3x cost: "
        + ", ".join(
            f"{name} (cost={c:.2f}, benefit={b:.2f}, "
            f"ratio={r})"
...
def test_genre_total_benefit_balanced(contracts):
    ...
    summary = ", ".join(
        f"{g}={totals[g]:.2f}" for g in genres
    )
    assert spread <= 1.0, (
        f"Genre benefit spread {spread:.2f} > 1.0: "
        f"{summary}"
    )
...
def _assert_genre_leads_resource(
    totals, genre, resource, margin=0.10,
):
    genre_val = totals[genre][resource]
    others = {
        g: totals[g][resource]
        for g in totals if g != genre
    }
